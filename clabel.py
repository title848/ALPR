...
#	k.append(mostcom[i][0])
	...
	#	if component not in pixel:
	#	    pixel.append(0)
	#	    pixel[component] += 1
	#	else : pixel[labels[x,y]] += 1
		outdata1[x,y] = colors[component]
		#if component == mostcom[2][0]:
		#	w.append(y)
    lbil = []
#    for x in range(width):
	#for y in range(w[0],w[0]+20):
	#	if w[0]+20 < width:
			#outdata[x,w[0]] = (0,255,0)
#			if (x,w[0]) in labels:
#				if labels[(x,w[0])] not in lbil and labels[(x,w[0])] != 0:
#					lbil.append(labels[(x,w[0])])
#    for (x,y) in labels:
#	if labels[(x,y)] in k and labels[(x,y)] in lbil:
#		if labels[(x,y)] not in abc:
#			abc.append(labels[(x,y)])
